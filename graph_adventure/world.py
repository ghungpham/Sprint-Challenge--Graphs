...
class World:
    ...
    def loadGraph(self, roomGraph):
        ...
        for roomID in roomGraph:
            ...
            if 'n' in roomGraph[roomID]:
                self.rooms[roomID].connectRooms('n', self.rooms[roomGraph[roomID]['n']])
            if 's' in roomGraph[roomID]:
                self.rooms[roomID].connectRooms('s', self.rooms[roomGraph[roomID]['s']])
            if 'e' in roomGraph[roomID]:
                self.rooms[roomID].connectRooms('e', self.rooms[roomGraph[roomID]['e']])
            if 'w' in roomGraph[roomID]:
                self.rooms[roomID].connectRooms('w', self.rooms[roomGraph[roomID]['w']])
        self.startingRoom = self.rooms[87]
    def printRooms(self):
        rotatedRoomGrid = []
